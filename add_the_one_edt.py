...
cur.execute("SELECT id FROM brands WHERE name='Dolce & Gabbana'")
row = cur.fetchone()
brand_id = row['id'] if row else None
if not brand_id:
    cur.execute("INSERT INTO brands (name) VALUES ('Dolce & Gabbana')")
    brand_id = cur.lastrowid

cur.execute("SELECT id FROM perfumes WHERE brand_id=? AND LOWER(name)=LOWER('The One') AND concentration='EDT'", (brand_id,))
if cur.fetchone():
    print("Já existe.")
else:
    cur.execute("INSERT INTO perfumes (brand_id, name, concentration, gender, family) VALUES (?,?,?,?,?)",
                (brand_id, 'The One', 'EDT', 'Masculino', 'Design'))
    pid = cur.lastrowid
    ...
    print(f"✔ Dolce & Gabbana — The One EDT adicionado (id={pid})")
