...
cur.execute("SELECT id FROM brands WHERE name='Ralph Lauren'")
row = cur.fetchone()
brand_id = row['id'] if row else None
if not brand_id:
    cur.execute("INSERT INTO brands (name) VALUES ('Ralph Lauren')")
    brand_id = cur.lastrowid
    print("✚ Marca Ralph Lauren criada")

cur.execute("SELECT id FROM perfumes WHERE brand_id=? AND LOWER(name)=LOWER('Polo Black')", (brand_id,))
if cur.fetchone():
    print("Já existe.")
else:
    cur.execute("INSERT INTO perfumes (brand_id, name, concentration, gender, family) VALUES (?,?,?,?,?)",
                (brand_id, 'Polo Black', 'EDT', 'Masculino', 'Design'))
    pid = cur.lastrowid
    ...
    print(f"✔ Ralph Lauren — Polo Black EDT adicionado (id={pid})")
