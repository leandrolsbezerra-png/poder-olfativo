...
cur.execute("SELECT id FROM perfumes WHERE brand_id=? AND LOWER(name)=LOWER('Omnia')", (brand_id,))
if cur.fetchone():
    print("Já existe.")
else:
    cur.execute("INSERT INTO perfumes (brand_id, name, concentration, gender, family) VALUES (?,?,?,?,?)",
                (brand_id, 'Omnia', 'EDP', 'Feminino', 'Design'))
    pid = cur.lastrowid
    ...
    print(f"✔ Bvlgari — Omnia EDP adicionado (id={pid})")
