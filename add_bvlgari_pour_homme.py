...
cur.execute("SELECT id FROM brands WHERE name='Bvlgari'")
row = cur.fetchone()
brand_id = row['id'] if row else None
if not brand_id:
    cur.execute("INSERT INTO brands (name) VALUES ('Bvlgari')")
    brand_id = cur.lastrowid

cur.execute("SELECT id FROM perfumes WHERE brand_id=? AND LOWER(name)=LOWER('Pour Homme')", (brand_id,))
if cur.fetchone():
    print("Já existe.")
else:
    cur.execute("INSERT INTO perfumes (brand_id, name, concentration, gender, family) VALUES (?,?,?,?,?)",
                (brand_id, 'Pour Homme', 'EDT', 'Masculino', 'Design'))
    pid = cur.lastrowid
    ...
    print(f"✔ Bvlgari — Pour Homme EDT adicionado (id={pid})")
